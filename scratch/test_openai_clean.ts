...
async function main() {
  ...
  try {
    ...
    if (final_output) {
      // Find the last '{' and last '}' to make sure we parse properly if there is any leak
      const startIdx = final_output.lastIndexOf('{');
      const endIdx = final_output.lastIndexOf('}');
      let cleanOutput = final_output;
      if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
        cleanOutput = final_output.substring(startIdx, endIdx + 1);
      }
      console.log("Parsed clean output:", JSON.parse(cleanOutput));
    }
  } catch (error) {
